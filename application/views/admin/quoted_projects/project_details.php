	...
	<div class="col-sm-6 col-lg-3">
		<div class="card p-3">
			<div class="d-flex align-items-center">
				...
				<div>
					<h5 class="mb-1">
						<p><?php echo $this->lang->line('xin_quote_date'); ?></p>
					</h5>
                      ...
                      <?php } ?><?php */ ?>
											<?php foreach ($company_ids as $cid) { ?>
												...
												<?php foreach ($ci_result as $re) { ?>
													<option value="<?php echo $re->user_id; ?>" <?php if (isset($_GET)) {
																																				if (in_array($re->user_id, $assigned_ids)) : ?> selected <?php endif;
																																																																								} ?>> <?php echo $re->first_name . ' ' . $re->last_name; ?></option>
												<?php } ?>
											<?php } ?>
										</select>
									</div>
