...
<!-- <script src="<?php echo base_url(); ?>skin/hrsale_vendor/assets/vendor/libs/select2/dist/js/select2.js"></script> new -->
...
<?php if (in_array($this->router->fetch_class(), ['purchasing', 'main'])) { ?>
...
<?php if (in_array($this->router->fetch_class(), ['purchase_requisitions', 'purchase_orders', 'purchase_deliveries', 'purchase_invoices'])) { ?>
	...
	<script>
		$(document).ready(function() {
			/* submit data */
			jQuery("#form_vendor").submit(function(e) {
				/*Form Submit*/
				e.preventDefault();
				var obj = jQuery(this),
		...
		function addVendor() {
			...
			$('[data-plugin="select_vendor"]').select2("close");
		}
	</script>


	<!-- include modal insert products -->
	<?php $this->load->view('admin/products/add_product'); ?>
	<script>
		$(document).ready(function() {
			/* submit data */
		...
		function addItemProduct() {
			$("#addItemProduct").modal("show");
			$('[data-plugin="select_item"]').select2("close");
		}
	</script>
<?php }; ?>