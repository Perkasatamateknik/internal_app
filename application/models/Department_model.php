...
class department_model extends CI_Model
{
	...
	// new row
	public function find_department_budget($query = false, $budget_id)
	{
		$this->db->where('budget_id', $budget_id);
		$budget_dep = $this->db->get("ms_finance_budgeting_department")->result();
		// dd($budget_dep);
		$not = [];
		foreach ($budget_dep as $r) {
			$not[] = $r->department_id;
		}

		if ($query) {
			$this->db->like('department_name', $query);
		}

		if (count($not) > 0) {
			$this->db->where_not_in('department_id', $not);
		}
