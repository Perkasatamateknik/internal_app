...
class Invoices extends MY_Controller
{
	...
	public function taxes_list()
	{
		...
		foreach ($taxes->result() as $r) {
			...
			// get type
			if ($r->type == 'fixed') : $type = $this->lang->line('xin_title_tax_fixed');
			else : $type = $this->lang->line('xin_title_tax_percent');
			endif;
			...
			if ($r->is_withholding == 1) {
				$withholding = '<span class="badge badge-success">' . $this->lang->line('ms_title_active') . '</span>';
			} else {
				$withholding = '<span class="badge badge-danger">' . $this->lang->line('ms_title_inactive') . '</span>';
			}
	...
	public function tax_read()
	{
		...
		$data = array(
			...
			'is_withholding' => $result[0]->is_withholding,
			'sell_account' => $result[0]->penjualan_account_id,
			'buy_account' => $result[0]->pembelian_account_id,
