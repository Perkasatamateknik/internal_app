$(function () {
	$('[data-plugin="select_account"]').select2({
		ajax: {
			delay: 250,
			url: site_url + "ajax_request/find_account_tax",
...
$(function () {
	$("#select_due_date").on("change", function () {
		...
		var startDate = new Date($("#publish_date").val());
...
$(document).ready(function () {
	...
	$("#ms_table").DataTable({
		ajax: {
			url: site_url + "finance/invoice/get_ajax_invoices/",
			data: {
				id: id,
			},
			type: "GET",
...
function addRow() {
	...
	var newRow = `<tr class="item-row" id="item-row-${rowCount}" data-id="${rowCount}">
		<td style="max-width:300px">
			<select class="form-control row_target_id" data-plugin="select_target_project" name="row_target_id[]" id="row_target_id_${rowCount}"value="0" data-placeholder="Select Project" required>
			</select>
		</td>
		<td>
	...
	$('[data-plugin="select_target_project"]').select2({
		ajax: {
			delay: 250,
...
$(document).ready(function () {
	$.ajax({
		url: site_url + "finance/invoices/get_view_details",
		type: "GET",
		dataType: "json",
		data: {
			id: $("#details").data("id"),
		...
		$.ajax({
			url: site_url + "finance/invoices/get_attachments",
			type: "GET",
			dataType: "json",
			data: {
				id: $("#details").data("id"),
